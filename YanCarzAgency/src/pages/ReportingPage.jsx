...
const ReportingPage = () => {
    ...
    const handleExport = (format) => {
        alert(`Exportation en cours au format ${format}...`);
    };
        ...
        <div style={{ animation: 'slideUpFade 0.4s ease' }}>
            <div className="page-header">
                <div>
                    ...
                    <p className="page-subtitle">Analyse des performances de votre agence</p>
                </div>
                <div className="flex gap-2">
                    <Button variant="outline" onClick={() => handleExport('PDF')}>
                        <div className="flex items-center gap-2"><Download size={16} /> PDF</div>
                    </Button>
                    <Button onClick={() => handleExport('Excel')}>
                        <div className="flex items-center gap-2"><Download size={16} /> Excel</div>
                    </Button>
                </div>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
                <Card title="Revenu Total" icon={<DollarSign size={20} color="var(--primary)" />}>
                    ...
                    <div className="flex items-center gap-1" style={{ fontSize: '0.75rem', color: '#10b981', marginTop: '0.25rem' }}>
                        <TrendingUp size={12} /> +12.4% <span style={{ color: 'var(--text-muted)' }}>vs mois dernier</span>
                    </div>
                </Card>
                <Card title="Réservations" icon={<Users size={20} color="#6366f1" />}>
                    <div style={{ fontSize: '1.5rem', fontWeight: 700, marginTop: '0.5rem' }}>{stats.totalBookings}</div>
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Total des locations</p>
                </Card>
                <Card title="Valeur Moyenne" icon={<TrendingUp size={20} color="#f59e0b" />}>
                    <div style={{ fontSize: '1.5rem', fontWeight: 700, marginTop: '0.5rem' }}>{stats.avgBookingValue.toLocaleString()} MAD</div>
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Par réservation</p>
                </Card>
                <Card title="Utilisation Flotte" icon={<Car size={20} color="#8b5cf6" />}>
                    <div style={{ fontSize: '1.5rem', fontWeight: 700, marginTop: '0.5rem' }}>{stats.fleetUtilization}</div>
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Taux d'occupation</p>
                </Card>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
                ...
                <Card title="Occupation de la Flotte">
                    <div style={{ width: '100%', height: 300, marginTop: '1rem' }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie
                                    data={fleetOccupancyData}
                                    innerRadius={60}
                                    outerRadius={80}
                                    paddingAngle={5}
                                    dataKey="value"
                                >
                                    {fleetOccupancyData.map((entry, index) => (
            ...
            <Card title="Top Véhicules Loués">
                            ...
                            <div key={i} className="flex items-center justify-between" style={{ padding: '0.75rem', borderRadius: 8, backgroundColor: 'var(--bg-panel)', border: '1px solid var(--border)' }}>
                                ...
                                <div className="flex items-center gap-8">
                                    <div className="text-right">
                                        <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>Locations</p>
