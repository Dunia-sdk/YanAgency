...
const BOOKING_COLS = [
    { key: 'id', label: 'Réf. Dossier', width: '15%', render: v => <span className="font-900 text-primary uppercase tracking-tight">#{v}</span> },
    { key: 'vehicle', label: 'Véhicule Loué', width: '25%', render: v => <div className="font-800 text-main uppercase text-[11px] tracking-tight">{v}</div> },
    { key: 'dates', label: 'Période', width: '25%', render: v => <div className="text-muted text-[10px] font-700 uppercase">{v}</div> },
    { key: 'total', label: 'Transaction', width: '15%', render: v => <span className="font-900 text-main tracking-tighter">{v.toLocaleString()} MAD</span> },
    { key: 'status', label: 'État', width: '20%', render: v => <span className={`badge badge-${v} font-900 text-[9px] uppercase tracking-[0.2em] px-3 py-1.5 shadow-sm`}>{v}</span> },
];
    ...
    if (!client) {
        return (
            <div className="flex flex-col items-center justify-center h-96">
                ...
                <p className="text-muted mb-4 font-900 uppercase tracking-[0.3em] text-[10px]">Identité Introuvable</p>
                <Button onClick={() => navigate('/clients')}>Retour à l'annuaire</Button>
            </div>
        );
    }
                        ...
                        <div className="relative z-10 flex flex-col items-center">
                            ...
                            <div className="flex flex-col gap-6 w-full text-left pt-10 border-t border-border/80">
                                {[
                                    ...
                                    { icon: <Phone size={18} />, label: 'Ligne Mobile', value: client.phone },
                                    { icon: <MapPin size={18} />, label: 'Zone Résidence', value: `${client.location}, Maroc` }
                                ].map((item, i) => (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="glass-panel p-8 border-l-[8px] border-success flex flex-col justify-between shadow-xl border-none">
                            <div className="flex justify-between items-center mb-6">
                                <div className="flex flex-col gap-1">
                                    <p className="text-muted text-[10px] font-900 uppercase tracking-[0.25em]">Volume Transactionnel</p>
                            ...
                            <div className="flex items-center justify-between pt-6 border-t border-border/50">
                                <div className="flex items-center gap-2">
                                    ...
                                    <p className="text-[10px] text-main font-900 uppercase tracking-widest">Compte en Règle</p>
                                </div>
                                <span className="text-[9px] font-800 text-muted/60 uppercase">Tier 1 Elite</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="glass-panel p-10 space-y-10 shadow-xl border-none">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-border/50 pb-8">
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    <h3 className="text-2xl font-900 text-main uppercase tracking-tighter">Historique de Collaboration</h3>
                                    <p className="text-[10px] text-muted font-800 uppercase tracking-widest mt-1">Extraction temps-réel du système</p>
                                </div>
                            </div>
                        ...
                        <div className="overflow-hidden rounded-3xl border border-border/30">
                            <Table
                                ...
                                data={history}
                                emptyMessage="Aucun historique d'activité disponible."
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="glass-panel p-10 flex flex-col shadow-xl border-none min-h-[480px]">
                            <h3 className="text-sm font-900 mb-10 flex items-center gap-3 text-main uppercase tracking-[0.2em] border-b border-border/50 pb-6">
                                <FileText size={20} className="text-primary" /> Documentation Client
                            </h3>
                            <div className="space-y-4">
                                {[
                                    { name: "Permis de Conduire", date: "VALIDE: 2028", color: "text-primary", details: "SCAN-A12" },
                                    { name: "Carte CIN Pro", date: "VÉRIFIÉ", color: "text-success", details: "CERT-33" },
                                    { name: "Attestation Domicile", date: "ACTUALISÉ", color: "text-muted", details: "DOC-99" }
                                ].map((doc, i) => (
                            ...
                            <div className="mt-auto pt-10">
                                <Button fullWidth variant="outline" className="h-14 font-900 text-[10px] tracking-[0.2em] uppercase border-dashed border-2 hover:bg-primary/5 text-primary">
                                    <Plus size={16} className="mr-2" /> AJOUTER DOCUMENT
                                </Button>
                            </div>
                        </div>
